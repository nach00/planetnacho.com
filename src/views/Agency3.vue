<template>
  <v-content>
    <div
      class="tinted-bg d-flex"
      style=""
    >
      <v-container>
        <v-container
          id="intro"
        >
          <div class="orange-divider mb-10 fancy-link" />
          <span class="fancy-link">Planet Nacho</span>
          provides digital solutions from concept to implementation by utilizing a design thinking approach. Our solutions focus on empathy and putting the user first. Research is always at the forefront of our process, and we believe in constant testing and validation through product iterations. Design solutions tailored to fit your company’s 2-week project management cycle.
        </v-container>

        <v-container
          id="info"
          class="tinted-bg d-none d-md-flex"
          style="top: 0"
        >
          <v-row>
            <v-col>
              <div
                class="
              cheese--text
              text-uppercase
              small-title
              fancy-link align-center d-flex"
              >
                <i
                  id="whirlybat"
                  style="font-size: 2em; text-transform: none"
                  class="wb-icon mr-3"
                >U</i>
                Current Availability
              </div>
              <div class="orange-divider mb-5" />
              <div style="display:flex; justify-content: space-between">
                <div class="font-weight-black mb-5">
                  Not accepting
                </div>
                <v-btn
                  outlined
                  color="white"
                  small
                  class="font-weight-black"
                  href="https://v2.waitwhile.com/welcome/planetnacho"
                >
                  Join waitlist
                </v-btn>
              </div>
              <div
                class="status"
              >
                We usually work on multiple projects at once, but we are not currently accepting new clients at the moment. Join the waitlist to get notified when we're ready to take on more work!
              </div>
            </v-col>
            <v-col>
              <div
                class="
              cheese--text
              text-uppercase
              fancy-link align-center d-flex small-title"
              >
                <i
                  id="whirlybat"
                  style="font-size: 2em; text-transform: none"
                  class="wb-icon mr-3"
                >l</i>
                Closed Alpha Testing
              </div>
              <div class="orange-divider mb-5" />
              <div class="">
                Alpha testing is now live! Be the first to help shape the future of our design process. Used to collect initial market research, generate product strategy, and gather client feedback.
              </div>
            </v-col>
            <v-col>
              <div
                class="
              cheese--text
              text-uppercase
              fancy-link align-center d-flex small-title"
              >
                <i
                  id="whirlybat"
                  style="font-size: 2em; text-transform: none"
                  class="wb-icon mr-3"
                >g</i>
                Pro Bono Services
              </div>
              <div class="orange-divider mb-5" />
              <div class="">
                We're happy to support your 501c3/Non-Profit organization! Animal/pet rescue organizations prioritized.
                <i
                  id="whirlybat"
                  style="font-size: 1em; text-transform: none"
                  class="wb-icon"
                >+</i>
              </div>
            </v-col>
          </v-row>
        </v-container>
        <v-container
          id="info"
          class="mt-5 d-md-none tinted-bg"
          fluid
        >
          <v-col>
            <div
              class="
              cheese--text
              text-uppercase
              fancy-link align-center d-flex"
            >
              <i
                id="whirlybat"
                style="font-size: 2em; text-transform: none"
                class="wb-icon mr-3"
              >U</i>
              Current Availability
            </div>
            <div class="orange-divider mb-5" />
            <div style="display:flex; justify-content: space-between">
              <div class="font-weight-black mb-5">
                Not accepting
              </div>
              <v-btn
                outlined
                color="white"
                small
                class="font-weight-black"
                href="https://v2.waitwhile.com/welcome/planetnacho"
              >
                Join waitlist
              </v-btn>
            </div>
            <div class="status mb-5">
              We usually work on multiple projects at once, but we are not currently accepting new clients at the moment. Join the waitlist to get notified when we're ready to take on more work!
            </div>
          </v-col>
          <v-col>
            <div
              class="
              cheese--text
              text-uppercase
              fancy-link align-center d-flex"
            >
              <i
                id="whirlybat"
                style="font-size: 2em; text-transform: none"
                class="wb-icon mr-3"
              >l</i>
              Closed Alpha Testing
            </div>

            <div class="orange-divider mb-5" />
            <div class="">
              Alpha testing is now live! Be the first to help shape the future of our design process. Used to collect initial market research, generate product strategy, and gather client feedback.
            </div>
          </v-col>
          <v-col>
            <div
              class="
              cheese--text
              text-uppercase
              fancy-link align-center d-flex"
            >
              <i
                id="whirlybat"
                style="font-size: 2em; text-transform: none"
                class="wb-icon mr-3"
              >g</i>
              Pro Bono Services
            </div>
            <div class="orange-divider mb-5" />
            <div class="">
              We're happy to support your 501c3/Non-Profit organization! Animal/pet rescue organizations prioritized.
              <i
                id="whirlybat"
                style="font-size: 1em; text-transform: none"
                class="wb-icon"
              >+</i>
            </div>
          </v-col>
        </v-container>
        <v-container
          id="content"
          class="mt-10"
        >
          <div
            class="
              cheese--text
              text-uppercase
              fancy-link align-center d-flex"
          >
            <i
              id="whirlybat"
              style="font-size: 2em; text-transform: none"
              class="wb-icon mr-3"
            >r</i>
            Service Packages
          </div>

          <div class="orange-divider mb-5" />

          <v-container class="d-none d-md-flex">
            <v-row style="border: white 2px solid; background: none">
              <v-col
                id="left"
                class=" d-flex flex-column"
              >
                <v-container class="text-uppercase">
                  <div class="font-weight-black mb-5">
                    <i
                      id="whirlybat"
                      style="font-size: 1em; text-transform: none"
                      class="wb-icon"
                    >E</i>
                    ON-DEMAND UX/UI MOCKUP SCREENS
                  </div>
                  <v-container>
                    <v-row class="justify-space-between">
                      <v-col class="cheese--text text-uppercase text-center darken-4 justify-center grey d-flex align-center">
                        <v-icon
                          class="cheese--text mr-1"
                        >
                          mdi-cellphone
                        </v-icon>
                        5 Mobile screens
                      </v-col>
                      <v-col class="or text-uppercase font-weight-black text-center d-flex align-center justify-center">
                        - or -
                      </v-col>
                      <v-col class="cheese--text text-uppercase text-center darken-4 grey ">
                        <v-icon
                          class="cheese--text mr-1"
                        >
                          mdi-monitor
                        </v-icon>
                        3 desktop screens
                      </v-col>
                    </v-row>
                  </v-container>
                </v-container>

                <v-container class="">
                  Custom professional mockup screens for mobile, tablet, or desktop devices that are ready to handoff to developers or for use in shareholder presentations.</span>
                </v-container>

                <v-container class="">
                  <i
                    id="whirlybat"
                    style="font-size: 1em; text-transform: none"
                    class="wb-icon mr-2"
                  >Π</i>
                  <span class="text-uppercase font-weight-black">Deliverables</span>
                  <ul class="">
                    <li>Finalized high-fidelity mockup mobile, tablet, or desktop screens</li>
                    <li>Original Adobe XD or Figma file</li>
                    <li>All associated assets (icons, logos, fonts, if applicable)</li>
                    <li>Developer handoff export link for style guide</li>
                    <li>Multiple options, alternatives, and wireframes worked out</li>
                    <li>Includes information architecture, user flows, and basic research</li>
                    <li>Clickthrough prototype and daily updates available upon request</li>
                  </ul>
                </v-container>
              </v-col>
              <v-col
                id="right"
                class=" d-flex flex-column"
                cols="4"
              >
                <v-container class="">
                  <v-container class=" d-flex justify-end pa-0 mb-5">
                    <span
                      class="text-uppercase font-weight-black"
                    >
                      Starting at
                    </span>
                    <span class="price cheese--text ml-1 font-weight-black">
                      $3K
                    </span>
                  </v-container>
                  <v-container
                    class="d-flex fluid justify-end pa-0 text-end"
                  >
                    2 week delivery
                    <v-icon
                      class="white--text ml-5"
                      small
                    >
                      mdi-clock
                    </v-icon>
                  </v-container>
                  <v-container class="d-flex fluid  text-end justify-end pa-0">
                    3 project meetings
                    <v-icon
                      class="white--text ml-5"
                      small
                    >
                      mdi-calendar
                    </v-icon>
                  </v-container>
                  <v-container class="d-flex fluid justify-end pa-0 text-end">
                    3 revisions
                    <v-icon
                      class="white--text ml-5"
                      small
                    >
                      mdi-pen
                    </v-icon>
                  </v-container>
                </v-container>
                <v-btn
                  align="end"
                  class="mt-auto cheese font-weight-black ma-3"
                  href="https://calendly.com/npradappet/jam"
                >
                  Let's jam!
                  <i
                    id="whirlybat"
                    style="font-size: 1em; text-transform: none"
                    class="wb-icon"
                  >ω</i>
                </v-btn>
              </v-col>
            </v-row>
          </v-container>

          <v-container class="d-md-none">
            <v-row style="border: white 2px solid; background: none">
              <v-col
                id="left"
                class=" d-flex flex-column"
              >
                <v-container class="text-uppercase">
                  <div class="font-weight-black mb-5">
                    <i
                      id="whirlybat"
                      style="font-size: 1em; text-transform: none"
                      class="wb-icon"
                    >E</i>
                    ON-DEMAND UX/UI MOCKUP SCREENS
                  </div>
                  <v-container>
                    <v-row class="justify-space-between align-center">
                      <v-col class="cheese--text text-uppercase text-center darken-4 grey">
                        <v-icon
                          class="cheese--text mr-1"
                        >
                          mdi-cellphone
                        </v-icon>
                        5 Mobile screens
                      </v-col>
                      <v-col class="or text-uppercase font-weight-black text-center">
                        - or -
                      </v-col>
                      <v-col class="cheese--text text-uppercase text-center darken-4 grey">
                        <v-icon
                          class="cheese--text mr-1"
                        >
                          mdi-monitor
                        </v-icon>
                        3 desktop screens
                      </v-col>
                    </v-row>
                  </v-container>
                </v-container>

                <v-container class="">
                  Custom professional mockup screens for mobile, tablet, or desktop devices that are ready to handoff to developers or for use in shareholder presentations.</span>
                </v-container>

                <v-container class="">
                  <i
                    id="whirlybat"
                    style="font-size: 1em; text-transform: none"
                    class="wb-icon mr-2"
                  >Π</i>
                  <span class="text-uppercase font-weight-black">Deliverables</span>
                  <ul class="">
                    <li>Finalized high-fidelity mockup mobile/tablet screens</li>
                    <li>Original Adobe XD or Figma file</li>
                    <li>All associated assets (icons, logos, fonts, if applicable)</li>
                    <li>Developer handoff export link for style guide</li>
                    <li>Multiple options, alternatives, and wireframes worked out</li>
                    <li>Includes information architecture, user flows, and basic research</li>
                    <li>Clickthrough prototype and daily updates available upon request</li>
                  </ul>
                </v-container>
              </v-col>

              <v-container class="">
                <v-container
                  class="d-flex fluid py-0"
                >
                  <v-icon
                    class="white--text mr-5"
                    small
                  >
                    mdi-clock
                  </v-icon>
                  2 week delivery
                </v-container>
                <v-container class="d-flex fluid py-0 ">
                  <v-icon
                    class="white--text mr-5"
                    small
                  >
                    mdi-calendar
                  </v-icon>
                  3 project meetings
                </v-container>
                <v-container class="d-flex fluid py-0">
                  <v-icon
                    class="white--text mr-5"
                    small
                  >
                    mdi-pen
                  </v-icon>
                  3 revisions
                </v-container>
              </v-container>
              <v-container class="d-flex">
                <v-container
                  class="d-flex"
                >
                  <span
                    class=""
                  >
                    Starting at
                  </span>
                  <span class="price cheese--text ml-1 font-weight-black">
                    $3K
                  </span>
                </v-container>
                <v-btn
                  align="end"
                  class=" cheese font-weight-black"
                  href="https://calendly.com/npradappet/jam"
                >
                  Let's jam!
                  <i
                    id="whirlybat"
                    style="font-size: 1em; text-transform: none"
                    class="wb-icon"
                  >ω</i>
                </v-btn>
              </v-container>
            </v-row>
          </v-container>
        </v-container>
      </v-container>
    </div>
  </v-content>
</template>
<style lang="sass">
//.body-2
//  font-size: medium !important
  //font-family: "Open Sans", sans-serif

.big-title
  font-size: medium !important
  //font-family: "Whirly Birdie", serif !important
  //font-variation-settings: "ital" 100, "wght" 100, "wdth" 100

.small-title
  font-size: small !important
  //font-family: "Whirly Birdie", serif !important
  //font-weight: 900 !important
</style>
